...
import { generateOtp, otpExpiry, otpValidate } from "../utils/otp.js";
import { sendOtpEmail } from "../config/mailer.js";
import jwt from "jsonwebtoken";

const makeToken = (id) => jwt.sign({ id }, process.env.JWT_SECRET, { expiresIn: "14d" });
const clean = (u) => ({
    _id: u._id,
    name: u.name,
    email: u.email,
...
export const verifyOtp = async (req, res) => {
    try {
        ...
        if (!user.isVerified && !otpValidate(user, otp))
        ...
        res.status(200).json({
            ...
            user: clean(user) // to exclude password and other sensitive data
        });
